...
export const getImmutableTypeHeight = (data) => {
  ...
  values.forEach((item) => {
    ...
    if (conversionTomID === '101') {
      // 图片
      height = height + typeHeight.image;
    } else if (conversionTomID === '102') {
      // 音频
      height = height + typeHeight.audio;
    } else if (conversionTomID === '103') {
      // 视频
      height = height + typeHeight.video;
    } else if (conversionTomID === '104') {
      // 商品
      height = height + typeHeight.goods;
    } else if (conversionTomID === '105') {
      // 问答
      // height = height + typeHeight.reward;
    } else if (conversionTomID === '106') {
      // 红包
      height = height + typeHeight.redPacket;
    } else if (conversionTomID === '107') {
      // 悬赏
      height = height + typeHeight.reward;
    } else if (conversionTomID === '108') {
      // 附件
      height = height + (typeHeight.file * item?.body?.length || 0);
    }
  });
  return height;
};
